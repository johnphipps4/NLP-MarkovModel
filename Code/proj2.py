...
""" runs viterbi algorithm for HMMs. Receives a transition probability dictionary,
    emission probability dictionary, unigram probability for tags (only used for
    first word), and the word sequence to predict the tags for.
...
def MEMMhelper(word, prev_bio_tag, bio_tag, pos_tag, weight_matrix, emCount, tagSeq, lexicon, pptag, nextword, nextpos, prevword, prevpos):
    new_sum = 0
    feature_arr = runFeats(word, pos_tag, prev_bio_tag, bio_tag, emCount, lexicon, nextword, nextpos, prevword, prevpos)
    for i in range(179):
...
def viterbiMEMM(word_seq, pos_seq, weights2D, unique_tags, emCount, trans_prob, lexicon):
    ...
    for tag in unique_tags:
        ...
        else: score[tag][0] = MEMMhelper(word_seq[0], "", tag, pos_seq[0], weights2D, emCount, unique_tags, lexicon, "",pos_seq[1], word_seq[1], "", "")
...
""" returns an array of all the outputs of each feature for the given inputs. """
def runFeats(word,tag,bio1,bio2,tagtok_2d,lexicon, nextword, nextpos, prevword, prevpos):
    ...
    # # feature 180 : tag transitionary probability
...
# MEMMguess = viterbifyMEMM(dev_sents, dev_pos_tags_ll, weights, unique_tags, emission_count_unk, bio_tags_transprob, lexicon)
...
# makecsv("testMEMM_good", MEMMtest)
